...
| 5 | Explicit links between related family trees | Complete (`v0.0.10`) | A parent, spouse, or sibling may live in another tree: the relative pickers search every tree, people from elsewhere are drawn as dashed stubs marked with the tree they belong to, and clicking one opens that tree. The relationship calculator and GEDCOM export still stop at the tree boundary, and now say so where it matters. |
